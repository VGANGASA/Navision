...
public class OpenBrowser {
	...
	/*
	 * public void capturescreenshot(ITestResult result){ if(result.getStatus() ==
	 * ITestResult.FAILURE){ String ss_path = ss.captureScreenshot(driver,
	 * result.getName()); String image = logger.addScreenCapture(ss_path);
	 * System.out.println("value for logger is : " + logger);
	 * logger.log(LogStatus.FAIL, "Title Verification",image);
	 * 
	 * } else if (result.getStatus() == ITestResult.SKIP){ String ss_path =
	 * ss.captureScreenshot(driver, result.getName()); String image =
	 * logger.addScreenCapture(ss_path); logger.log(LogStatus.SKIP,
	 * "Title Verification",image);
	 * 
	 * }else if(result.getStatus() == ITestResult.SUCCESS){ String ss_path =
	 * ss.captureScreenshot(driver, result.getName()); String image =
	 * logger.addScreenCapture(ss_path); logger.log(LogStatus.PASS,
	 * "Title Verification",image); }
	 * 
	 * report.endTest(logger); report.flush(); }
	 * 
	 * public void startTest(){ report = new
	 * ExtentReports("../Navision/target/extentreport.html");
	 * 
	 * // report = new ExtentReports(System.getProperty("user.dir")+
	 * "/test-output/coreproduct.html",true); report.addSystemInfo("Host Name",
	 * "Navision DevOps")
